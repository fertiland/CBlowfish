...
int main (int argc, char *argv[])
{
  ...
  if ( enc )
  {
    ...
    //Make sure the input length is the multiple of 8 bytes. 
    int plaintextLen = strlen ((char *)plaintext);
    int inputLen = ( ( plaintextLen + 7) / 8 ) * 8;
    char inputText[inputLen];
    int i, ii;
    ...
    for(ii = plaintextLen; ii < inputLen ; ii ++)
    {
        inputText[ii]=0;
    }
    ciphertext_len = encrypt (inputText, inputLen, key, iv, ciphertext);
